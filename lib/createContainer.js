...
module.exports = function createContainer(options) {
  ...
  // These are passed to all registration functions.
  ...
  const makeRegisterFunction = (registrationFactory) => {
    ...
    return (name, value, opts) => {
      ...
      opts = Object.assign(
        {},
        defaultOptions,
        // If the name was an object, then the 2nd parameter
        // will be the options.
        isPlainObject(name) ? value : opts
      );

      Object.keys(obj).forEach(
        key => register(registrationFactory(key, obj[key], opts))
      );
      return container;
    };
  };
