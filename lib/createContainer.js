...
const { last, nameValueToObject, isClass, deprecated } = require('./utils')
...
module.exports = function createContainer(options, __parentContainer) {
   ...
   */
  const rollUpRegistrations = bustCache => {
    if (__computedRegistrations && !bustCache) {
      return __computedRegistrations
    }

    __computedRegistrations = Object.assign(
      {},
      __parentContainer && __parentContainer[ROLL_UP_REGISTRATIONS](bustCache),
      registrations
    )
  ...
  container[ROLL_UP_REGISTRATIONS] = rollUpRegistrations
   ...
   */
  const makeRegister = (fnName, fn, verbatimValue) =>
    function registerShortcut(name, value, opts) {
      deprecated(
        `The register${
          fnName
        } and other "register*" shortcuts have been deprecated and will be removed in v3.\nUse "container.register({ name: as${
          fnName
        }(target) })" instead. See https://github.com/jeffijoe/awilix/issues/60`
      )
   ...
   * The container
   */
  container.registerFunction = makeRegister('Function', asFunction)
   ...
   * The container
   */
  container.registerClass = makeRegister('Class', asClass)
   ...
   * The container
   */
  container.registerValue = makeRegister(
    'Value',
    asValue,
    /* verbatimValue: */ true
  )
