...
const asFunction = (fn, opts) => {
  ...
  const resolve = generateResolve(fn, fn, true)
  const result = {
    resolve,
...
const asClass = (Type, opts) => {
  ...
  const resolve = generateResolve(buildNewClassFunction(Type), Type.prototype.constructor)
  const result = {
    resolve,
    lifetime: opts.lifetime,
    resolutionMode: opts.resolutionMode
 ...
 */
function generateResolve (fn, depsFn, isFunction) {
  ...
  return function (container) {
    ...
    if (resolutionMode === ResolutionModes.PROXY) {
      ...
      let dependencies = getDependencies(depsFn)

      // We have dependencies so we need to resolve them manually
      if (dependencies.length > 0) {
        let children = []
        dependencies.forEach((d) => children.push(container.cradle[d]))
        // We need to test this because of the nature of the spread operator (...) when passed to another function.
        // If I didn't want to be fancy and move the object construction into a function then we may not need this.
        // It produces less code in the long-run, though.
        return isFunction ? fn(...children) : fn([...children])
      }
      return fn()
    }
  }
