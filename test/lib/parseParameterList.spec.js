...
describe('parseParameterList', function () {
  ...
  it('supports regular functions', function () {
    ...
    expect(
      parseParameterList(function hello () {}.toString())
    ).to.deep.equal([])
    expect(
      parseParameterList(function () {}.toString())
    ).to.deep.equal([])
  })
  ...
  it('supports arrow functions', function () {
    expect(
      parseParameterList(((dep1, dep2) => {}).toString())
    ).to.deep.equal(['dep1', 'dep2'])
    expect(
      parseParameterList(((dep1, dep2) => 42).toString())
    ).to.deep.equal(['dep1', 'dep2'])
    expect(
      parseParameterList(((dep1, dep2, dep3) => {}).toString())
    ).to.deep.equal(['dep1', 'dep2', 'dep3'])
    expect(
      parseParameterList((() => {}).toString())
    ).to.deep.equal([])
    expect(
      parseParameterList((() => 42).toString())
    ).to.deep.equal([])
    expect(
      parseParameterList((dep1 => expect).toString())
    ).to.deep.equal(['dep1'])
  })
