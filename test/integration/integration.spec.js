...
describe('integration tests', function() {
  ...
  it('registered all services as scoped', function() {
    ...
    expect(container.registrations.mainService.lifetime).to.equal(
      Lifetime.SCOPED
    )
    expect(container.registrations.anotherService.lifetime).to.equal(
      Lifetime.SCOPED
    )
    expect(container.registrations.answerRepository.lifetime).to.equal(
      Lifetime.TRANSIENT
    )

    expect(scope1.resolve('mainService')).to.equal(
      scope1.resolve('mainService')
    )
    expect(scope1.resolve('mainService')).to.not.equal(
      scope2.resolve('mainService')
    )

    expect(scope1.resolve('anotherService')).to.equal(
      scope1.resolve('anotherService')
    )
    expect(scope1.resolve('anotherService')).to.not.equal(
